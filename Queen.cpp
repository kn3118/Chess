...
Queen::Queen(Colour col) : Piece(col) {}


Queen::~Queen() {}


string Queen::get_piece() { return (piece_colour == White ? "♕" : "♛" ); }


string Queen::get_name_piece() { return "Queen"; }


bool Queen::piece_valid_move(string source_square, string dest_square, Board board, int& error_code) {
  ...
  if (abs(file_change) == abs(rank_change)) {
    if ((file_change > 0) && (rank_change > 0)) {
      for (int i = source_file + 1; i < dest_file; i++) {
	path_file = string(1, i);
	tmp_path_rank += 1;
